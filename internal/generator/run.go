...
func Run(cfg *config.Config) (returnErr error) {
	...
	if err != nil {
		return fmt.Errorf("failed to parse message files from pattern %q:\n  %w\n\nSuggestions:\n  - Check that message files exist and have valid YAML syntax\n  - Verify glob pattern matches your file structure\n  - Ensure templates don't exceed complexity limits", cfg.MessagesGlob, err)
	}

	placeholders, err := parser.ParsePlaceholders(cfg.PlaceholdersGlob, cfg.Locales, cfg.Compound)
	if err != nil {
		return fmt.Errorf("failed to parse placeholder files from pattern %q:\n  %w\n\nSuggestions:\n  - Check that placeholder files have valid YAML syntax\n  - Verify placeholder names are valid Go identifiers\n  - Ensure all specified locales (%v) have corresponding values", cfg.PlaceholdersGlob, err, cfg.Locales)
	}

	// Validate that we have messages after parsing
	if len(messages) == 0 {
		return fmt.Errorf("no messages found after parsing pattern %q\n\nSuggestions:\n  - Check that message files exist in the specified location\n  - Verify the glob pattern is correct\n  - Ensure message files contain valid message definitions", cfg.MessagesGlob)
	}

	defs, err := model.Build(messages, placeholders, cfg.Locales)
	if err != nil {
		return fmt.Errorf("failed to build models from parsed data:\n  %w\n\nSuggestions:\n  - Check for placeholder type mismatches\n  - Verify all message templates reference valid placeholders\n  - Ensure suffix notation is used correctly for multiple instances", err)
	}

	if err := os.MkdirAll(cfg.OutputDir, 0755); err != nil {
		return fmt.Errorf("failed to create output directory %q: %w\n\nSuggestions:\n  - Check directory permissions\n  - Ensure parent directories exist\n  - Verify the path is not read-only", cfg.OutputDir, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to build templates:\n  %w\n\nSuggestions:\n  - Check for missing placeholder definitions\n  - Verify template syntax is valid\n  - Ensure all referenced placeholders exist", err)
	}
	...
	if err := templatex.Render(
		...
		return fmt.Errorf("failed to render generated code to %q:\n  %w\n\nSuggestions:\n  - Check output directory permissions\n  - Verify package name is valid\n  - Ensure templates generate valid Go code\n  - Check for disk space availability", outputFile, err)
	}
